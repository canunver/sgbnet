...
namespace TasinirMal
{
    ...
    /// </summary>
    public partial class EnCokKullanilanlarEski : TMMSayfa
    {
        /// <summary>
        ...
        {
            ddlDonem.Items.Clear();
            for (int i = 1; i <= 4; i++)
                ddlDonem.Items.Add(new ListItem(string.Format(Resources.TasinirMal.FRMECK002, i.ToString()), i.ToString()));
        }
        ...
        private DepoDurum KriterTopla()
        {
            ...
            kriter.ilKod = ddlIl.SelectedValue.Trim();
            kriter.ilAd = ddlIl.SelectedItem.Text.Trim();
            kriter.ilceKod = ddlIlce.SelectedValue.Trim();
            kriter.ilceAd = ddlIlce.SelectedItem.Text.Trim();
            kriter.hesapPlanKod = txtHesapPlanKod.Text.Trim();
            kriter.hesapPlanAd = lblHesapPlanAd.Text.Trim();
            kriter.donem = OrtakFonksiyonlar.ConvertToInt(ddlDonem.SelectedValue, 0);

            //Detay raporu için
            kriter.raporTur = OrtakFonksiyonlar.ConvertToInt(rblRaporTur.SelectedValue, 0);
        ...
        private void EnCokKullanilanlarRaporu(DepoDurum kriter)
        {
            ...
            for (int i = 0; i < bilgi.objeler.Count; i++)
            {
                ...
                else if (yazilanHesapKod != depo.hesapPlanKod.Substring(0, 3))
                {
                    ...
                    if (yazilanHesapKod == ((int)ENUMTasinirHesapKodu.MAKINECIHAZ).ToString())
                        index = 1;
                    else if (yazilanHesapKod == ((int)ENUMTasinirHesapKodu.TASIT).ToString())
                        index = 2;
                    else if (yazilanHesapKod == ((int)ENUMTasinirHesapKodu.DEMIRBAS).ToString())
                        index = 3;
            ...
            *******************/
            ...
            for (int i = kaynakSatir + 1; i <= satir; i++)
            {
                ...
                if (hesapKod == ((int)ENUMTasinirHesapKodu.TUKETIM).ToString())
                    index = 0;
                else if (hesapKod == ((int)ENUMTasinirHesapKodu.MAKINECIHAZ).ToString())
                    index = 1;
                else if (hesapKod == ((int)ENUMTasinirHesapKodu.TASIT).ToString())
                    index = 2;
                else if (hesapKod == ((int)ENUMTasinirHesapKodu.DEMIRBAS).ToString())
                    index = 3;
